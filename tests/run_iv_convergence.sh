...
# EDIT THESE PARAMS
mesh="${laghos_dir}/../Laglos/data/square-vortex.mesh"
final_time=1.
cfl=0.5
ot=1
ok=2 # must be ot + 1
oq=4 # must be >= 4
solver_type=14
output_location="${results_dir}/tests/ot1/iv"
output_file="${output_location}/out-iv-r"
########

options="-m ${mesh} -fa -p 9 -tf ${final_time} -cfl ${cfl} "
...
# ./laghos ${options} -rs 10 > ${output_file}10 &
